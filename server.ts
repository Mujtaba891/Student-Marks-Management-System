...
async function startServer() {
  ...
  app.post('/api/gemini/extract-marksheet', async (req, res) => {
    try {
      const { mimeType, base64Data, fileName, isMultiPage } = req.body;
...
5. Return strictly valid JSON following the schema.`;
      ...
      // Candidate models with fallback hierarchy to guard against 503 high-demand spikes
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            ...
            if (isRetryable && attempt < 2) {
              // Wait 1.2s + jitter before retrying same model
              const delay = 1200 * attempt + Math.floor(Math.random() * 400);
              await new Promise(r => setTimeout(r, delay));
            } else if (!isRetryable) {
              // Non-retryable error for this model, break to try next model or fail
              break;
            }
          }
        }
  ...
  if (Array.isArray(data.rows)) {
    for (const rawRow of data.rows) {
      if (Array.isArray(rawRow)) {
        // 2D Array format: items in rawRow match headers by index
