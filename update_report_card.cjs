...
export async function generateReportCardPDF(student: Student, results: Result[]) {
  ...
  // Try to fetch Logo
  const logoBase64 = await getBase64ImageFromUrl('/GDC-logo.png');
  ...
  if (logoBase64) {
    ...
    textStartX = 120; // Shift text slightly to the right if logo exists, or center it anyway
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Roll Number:', 125, startY + 2);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Academic Session:', 125, startY + 12);
  doc.setFont('helvetica', 'normal');
